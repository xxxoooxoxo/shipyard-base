...
// tab values. Radix requires strings, so numbers are coerced on the way in and
// converted back in onValueChange when the call site used numeric values.
function Tabs({
  className,
  ...
  return (
    <TabsPrimitive.Root
      ...
      onValueChange={
        onValueChange &&
        ((next: string) =>
          (onValueChange as (value: string | number) => void)(
            isNumeric ? Number(next) : next
          ))
      }
      {...props}
    />
